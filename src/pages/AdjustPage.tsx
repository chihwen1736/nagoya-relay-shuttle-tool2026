        ...
        <div className="bg-white rounded-xl shadow overflow-x-auto">
          <table className="w-full text-sm">
            <thead className="bg-gray-50 text-gray-500">
              <tr>
                <th className="text-left px-2 py-2">車次</th>
                <th className="text-left px-2 py-2">方向</th>
                <th className="text-left px-2 py-2">出發時間</th>
                <th className="text-left px-2 py-2">上車地點代碼</th>
                <th className="text-left px-2 py-2">下車地點代碼</th>
                <th className="text-left px-2 py-2">人數</th>
                <th className="text-left px-2 py-2">車輛</th>
                <th className="text-left px-2 py-2">隊伍／單號</th>
                <th className="text-left px-2 py-2">來源</th>
                <th className="text-left px-2 py-2">備註</th>
                <th className="text-left px-2 py-2">指派結果</th>
            ...
            <tbody>
              {trips.map((t) => (
                <tr
                  key={t.id}
                  className={`border-t ${
                    t.assignment_status === "unassigned" ? "bg-red-50" : t.assignment_status === "conflict" ? "bg-yellow-50" : ""
                  }`}
                >
                  <td className="px-2 py-2 font-mono">{t.trip_no}</td>
                  <td className="px-2 py-2">{LEG_TYPE_LABELS[t.leg_type]}</td>
                  ...
                  <td className="px-2 py-2 text-xs text-gray-500">{TRIP_ORIGIN_LABELS[t.origin]}</td>
                  <td className="px-2 py-2">
                    <input
                      value={t.notes}
          ...
          <p className="text-xs text-gray-400 p-2">
            提示：地點/車輛/上下車地點清單來自您匯入的地點資料與車輛資料；「上車地點」若沒有對應到中繼站/場館代碼，可用地點資料補齊後重新匯入。
          </p>
        </div>
      )}
    </div>
